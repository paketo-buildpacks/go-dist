...
func testVersions(t *testing.T, context spec.G, it spec.S) {
	...
	context("FindNewVersions", func() {
		it("returns a list of all new versions", func() {
			newVersions, err := alternative.FindNewVersions(buildpackTOMLPath, []string{"1.2.2", "1.2.3", "1.2.4", "2.3.4", "2.3.5", "2.3.6", "9.9.9"})
			Expect(err).NotTo(HaveOccurred())
	...
	context("failure cases", func() {
		context("the buildpack cannot be parsed", func() {
			...
			it("returns an error", func() {
				_, err := alternative.FindNewVersions(buildpackTOMLPath, []string{"1.2.2", "1.2.3", "1.2.4", "2.3.4", "2.3.5", "2.3.6", "9.9.9"})
